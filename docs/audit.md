...
| `src/environment.h` | `sample_direction` | La rotation (`set_rotation`) semblait ignorée (selon commentaires). | **Résolu** : Les commentaires étaient obsolètes. La logique `phi - rotation` est bien présente. | **Fixed**: Commentaires nettoyés. |
