...
using Real = double; // Use double to match Python default and avoid casting issues
...
struct Ray {
    ...
    Ray(const Vec3& origin, const Vec3& direction, Real time = 0.0, bool shadow = false, bool primary = false)
        : orig(origin), dir(direction), tm(time), is_shadow(shadow), is_primary(primary) {}
...
public:
    ...
    virtual Vec3 emit(const Ray& r_in, const HitRecord& rec, Real u, Real v, const Vec3& p) const {
        return Vec3(0,0,0);
    }
...
public:
    ...
    virtual bool scatter(const Ray& r_in, const HitRecord& rec, ScatterRecord& srec) const override {
        ...
        srec.specular_ray = Ray(rec.p, unit_vector(rec.normal + random_unit_vector()), r_in.tm); // Direction is stored here
        return true;
    }

    virtual Real scattering_pdf(const Ray& r_in, const HitRecord& rec, const Ray& scattered) const override {
...
public:
    ...
    LambertianChecker(const Vec3& a1, const Vec3& a2, Real s) : albedo1(a1), albedo2(a2), scale(s) {}

    virtual bool scatter(const Ray& r_in, const HitRecord& rec, ScatterRecord& srec) const override {
        srec.is_specular = false;
        
        // Checkerboard pattern
        // We use world position, but we should potentially use texture coordinates u,v?
        // For simplicity on a sphere, sticking to spatial sine pattern is classic "One Weekend" style.
        // Using p directly works for general objects.
        
        Real eps = 1e-5; // avoid artifacts at 0
        Real sines = std::sin(scale * rec.p.x()) * std::sin(scale * rec.p.y()) * std::sin(scale * rec.p.z());
        
        if (sines < 0) srec.attenuation = albedo1;
        else srec.attenuation = albedo2;

        srec.specular_ray = Ray(rec.p, unit_vector(rec.normal + random_unit_vector()), r_in.tm);
        return true;
    }
...
public:
    ...
    virtual bool scatter(const Ray& r_in, const HitRecord& rec, ScatterRecord& srec) const override {
        ...
        srec.specular_ray = Ray(rec.p, reflected + fuzz * random_in_unit_sphere(), r_in.tm);
        srec.attenuation = albedo;
...
public:
    ...
    virtual bool scatter(const Ray& r_in, const HitRecord& rec, ScatterRecord& srec) const override {
        ...
        srec.specular_ray = Ray(rec.p + 0.001*r_in.dir, r_in.dir, r_in.tm, r_in.is_shadow, r_in.is_primary); 
        
        srec.attenuation = Vec3(1.0, 1.0, 1.0);
        return true; 
    }

    virtual Vec3 emit(const Ray& r_in, const HitRecord& rec, Real u, Real v, const Vec3& p) const override {
        if (r_in.is_shadow) {
            return emit_color;
        }
        return Vec3(0,0,0);
...
public:
    ...
    Dielectric(Real index_of_refraction, const Vec3& tint_color = Vec3(1.0, 1.0, 1.0)) 
...
public:
    ...
    virtual bool scatter(const Ray& r_in, const HitRecord& rec, ScatterRecord& srec) const override {
        ...
        if (random_real() < reflect_prob) {
             ...
             // Diffuse Base
             // Treat as specular to avoid PDF issues for now (Path Tracing handles it fine)
             srec.is_specular = true;
             srec.attenuation = albedo;
...
Vec3 Sphere::random(const Vec3& o) const {
    ...
    auto u = unit_vector(cross(a, w)); // A bit different from python but basis is arbitrary
    // Correct way: use consistent UP
    if (std::abs(w.x()) > 0.9) a = Vec3(0,1,0); else a = Vec3(1,0,0);
    u = unit_vector(cross(a, w)); // wait, if w is (1,0,0), a=(0,1,0), cross is (0,0,1). OK.
    // Re-check logic
    Vec3 v = cross(w, u);
...
bool Quad::hit(const Ray& r, Real t_min, Real t_max, HitRecord& rec) const {
    ...
    if (!((0 <= alpha && alpha <= 1) && (0 <= beta && beta <= 1))) return false;
...
bool HittableList::hit(const Ray& r, Real t_min, Real t_max, HitRecord& rec) const {
    HitRecord temp_rec;
...
Vec3 HittableList::random(const Vec3& o) const {
    ...
    int idx = dist(generator) % int_size;
    // Fix: dist(gen) range issue
    std::uniform_int_distribution<int> range_dist(0, int_size-1);
    return objects[range_dist(generator)]->random(o);
}
...
struct EnvironmentMap {
    ...
    EnvironmentMap(const std::vector<Real>& d, int w, int h) : data(d), width(w), height(h) {}
    ...
    Vec3 sample(const Vec3& dir, bool is_primary) const {
        if (dir.length_squared() < 1e-8) return Vec3(0,0,0);
        
        Real strength = is_primary ? visible_strength : lighting_strength;
        if (strength <= 0) return Vec3(0,0,0);
        ...
        auto get_pixel = [&](int x, int y) {
            ...
            if (idx < 0 || idx + 2 >= data.size()) return Vec3(0,0,0);
...
Vec3 ray_color(const Ray& r, const Hittable& world, const HittableList& lights, const EnvironmentMap* env_map, int depth) {
    if (depth <= 0) return Vec3(0,0,0);

    HitRecord rec;
    if (!world.hit(r, 0.001, std::numeric_limits<Real>::infinity(), rec)) {
        if (env_map) return env_map->sample(r.dir, r.is_primary);
        return Vec3(0,0,0);
    }
    ...
    if (srec.is_specular) {
        return emitted + srec.attenuation * ray_color(srec.specular_ray, world, lights, env_map, depth-1);
    }
    ...
    if (!lights.objects.empty()) {
        ...
        if (light_pdf_val > 0) {
            HitRecord hit_light;
            if (world.hit(light_ray, 0.001, std::numeric_limits<Real>::infinity(), hit_light)) {
                 auto li_emit = hit_light.mat_ptr->emit(light_ray, hit_light, hit_light.u, hit_light.v, hit_light.p);
                 if (li_emit.length_squared() > 0) {
                     auto scattering_pdf = rec.mat_ptr->scattering_pdf(r, rec, light_ray);
                     if (scattering_pdf > 0) {
                         direct_light = li_emit * srec.attenuation * scattering_pdf / light_pdf_val;
                     }
                 }
            }
        }
    ...
    // 2. Indirect
    auto indirect = srec.attenuation * ray_color(srec.specular_ray, world, lights, env_map, depth-1); 
    
    return emitted + direct_light + indirect;
}

// Forward decl
Vec3 ray_color_nee(const Ray& r, const Hittable& world, const HittableList& lights, const EnvironmentMap* env_map, int depth, bool allow_emission = true);

Vec3 ray_color_nee(const Ray& r, const Hittable& world, const HittableList& lights, const EnvironmentMap* env_map, int depth, bool allow_emission) {
    if (depth <= 0) return Vec3(0,0,0);

    HitRecord rec;
    if (!world.hit(r, 0.001, std::numeric_limits<Real>::infinity(), rec)) {
        if (env_map && allow_emission) return env_map->sample(r.dir, r.is_primary);
        return Vec3(0,0,0);
    }

    Vec3 emitted = rec.mat_ptr->emit(r, rec, rec.u, rec.v, rec.p);
    if (!allow_emission) emitted = Vec3(0,0,0); // Suppress
    ...
    if (srec.is_specular) {
        return emitted + srec.attenuation * ray_color_nee(srec.specular_ray, world, lights, env_map, depth-1, true);
    }
    ...
    if (!lights.objects.empty()) {
        ...
        if (light_pdf > 0) {
            ...
            for (int i = 0; i < 5; ++i) { // Max 5 transparent layers
                if (world.hit(shadow_ray, 0.001, std::numeric_limits<Real>::infinity(), hit_light)) {
                     // Hit something
                     if (hit_light.mat_ptr->is_transparent()) {
                         // Attenuate
                         ScatterRecord srec_shadow;
                         // Hack: use scatter to get attenuation? 
                         // For Dielectric, scatter gives us tint in attenuation if we force it?
                         // Actually, Dielectric::scatter returns tint in attenuation.
                         if (hit_light.mat_ptr->scatter(shadow_ray, hit_light, srec_shadow)) {
                             transmission = transmission * srec_shadow.attenuation;
                         }
                         
                         // Continue ray through
                         shadow_ray = Ray(hit_light.p + 0.001*shadow_ray.dir, shadow_ray.dir, shadow_ray.tm, true);
                     } else {
                         // Opaque or Light?
                         // Check if it's a light source
                         auto li_emit = hit_light.mat_ptr->emit(shadow_ray, hit_light, hit_light.u, hit_light.v, hit_light.p);
                         if (li_emit.length_squared() > 0) {
                             // It is a light!
                             // Use this emission
                             auto scattering_pdf = rec.mat_ptr->scattering_pdf(r, rec, light_ray);
    ...
    // Indirect
    // Use stored specular_ray (which is diffuse importance sampled ray in this case)
    Vec3 indirect = srec.attenuation * ray_color_nee(srec.specular_ray, world, lights, env_map, depth-1, false);
...
public:
    ...
    Camera(Vec3 lookfrom, Vec3 lookat, Vec3 vup, Real vfov, Real aspect_ratio, Real aperture, Real focus_dist) {
    ...
    Ray get_ray(Real s, Real t) const {
        ...
        return Ray(origin + offset, lower_left_corner + s*horizontal + t*vertical - origin - offset, random_real());
    }
};
...
public:
    ...
    void add_sphere(const Vec3& center, Real radius, std::string mat_type, const Vec3& color, Real fuzz=0, Real ir=1.5) {
        std::shared_ptr<Material> mat;
        if (mat_type == "lambertian") mat = std::make_shared<Lambertian>(color);
        else if (mat_type == "metal") mat = std::make_shared<Metal>(color, fuzz);
        else if (mat_type == "dielectric") mat = std::make_shared<Dielectric>(ir, color);
        else if (mat_type == "plastic") mat = std::make_shared<Plastic>(color, ir, fuzz);
        else if (mat_type == "light") mat = std::make_shared<DiffuseLight>(color);
        
        auto sphere = std::make_shared<Sphere>(center, radius, mat);
        world.add(sphere);
        if (mat_type == "light") lights.add(sphere);
    }
    
    void add_checker_sphere(const Vec3& center, Real radius, const Vec3& color1, const Vec3& color2, Real scale) {
        auto mat = std::make_shared<LambertianChecker>(color1, color2, scale);
        auto sphere = std::make_shared<Sphere>(center, radius, mat);
        world.add(sphere);
    }

    void add_invisible_sphere_light(const Vec3& center, Real radius, const Vec3& color) {
    ...
    void add_quad(const Vec3& Q, const Vec3& u, const Vec3& v, std::string mat_type, const Vec3& color, Real fuzz=0, Real ir=1.5) {
        std::shared_ptr<Material> mat;
        if (mat_type == "lambertian") mat = std::make_shared<Lambertian>(color);
        else if (mat_type == "metal") mat = std::make_shared<Metal>(color, fuzz);
        else if (mat_type == "dielectric") mat = std::make_shared<Dielectric>(ir, color);
        ...
        world.add(quad);
        if (mat_type == "light") lights.add(quad);
    }
    
    void set_camera(const Vec3& lookfrom, const Vec3& lookat, const Vec3& vup, Real vfov, Real aspect, Real aperture, Real dist) {
        camera = std::make_shared<Camera>(lookfrom, lookat, vup, vfov, aspect, aperture, dist);
    }

    void set_environment(nb::object image) {
        PyObject* obj = image.ptr();
        Py_buffer view;
        int flags = PyBUF_STRIDES | PyBUF_FORMAT | PyBUF_ND; // Request strides and format
        
        if (PyObject_GetBuffer(obj, &view, flags) != 0) {
            throw std::runtime_error("Argument is not a buffer compatible object (numpy array required)");
        }
        ...
        if (view.ndim != 3) throw std::runtime_error("Environment map must be 3D array (H,W,C)");
        
        size_t h = static_cast<size_t>(view.shape[0]);
        size_t w = static_cast<size_t>(view.shape[1]);
        size_t c = static_cast<size_t>(view.shape[2]);
        
        if (c != 3) throw std::runtime_error("Environment map must have 3 channels (RGB)");

        // Assume float32 ('f') or double ('d') or check view.format
        // But for robustness just assume float* access for now or check itemsize
        if (view.itemsize != 4) {
             // throw std::runtime_error("Environment map must be float32 (itemsize 4)");
             // Relaxed: maybe user passed something else? But we proceed assuming float.
        }
        ...
        for (size_t y = 0; y < h; ++y) {
            for (size_t x = 0; x < w; ++x) {
                for (size_t k = 0; k < 3; ++k) {
                    const void* pixel_addr = buf + y * stride_y + x * stride_x + k * stride_c;
    ...
    nb::ndarray<nb::numpy, float> render(int width, int height, int spp, int depth, int n_threads) {
        // Init progress
                ...
                for (int j = 0; j < height; ++j) {
                    for (int i = 0; i < width; ++i) {
                        ...
                        for (int s = 0; s < spp; ++s) {
                            ...
                            r.is_primary = true;
                            pixel_color += ray_color_nee(r, world, lights, background.get(), depth, true);
                        }
        ...
        return nb::ndarray<nb::numpy, float>(data, { (size_t)height, (size_t)width, 3ul }, owner);
    }
